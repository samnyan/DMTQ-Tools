...
public sealed class PatchPackageValidator(PatchChecksumService checksumService)
{
    ...
    {
        var actualSize = checksumService.GetFileSize(filePath);
        if (actualSize != expectedSize)
        {
            result.Errors.Add($"Size mismatch for {relativePath}: expected {expectedSize}, actual {actualSize}");
        }

        var actualChecksum = await checksumService.ComputeMd5Async(filePath, cancellationToken).ConfigureAwait(false);
